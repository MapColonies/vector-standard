...
import { OPENAPI_SPEC, PREFIX_HEADER, ROOT_HEADER, SERVICES } from '@common/constants';
import { toPublicSpec } from '@common/openapi';
...
const docsRouterFactory: FactoryFunction<Router> = (dependencyContainer) => {
  ...
  const spec = dependencyContainer.resolve<OpenapiSpec>(OPENAPI_SPEC);

  const viewerRouter = new OpenapiViewerRouter({ ...openapiConfig, filePathOrSpec: spec });
  viewerRouter.setup();

  const usePublicSpec: RequestHandler = (req, _, next) => {
